...
2. **1–10 Security Questions** — Specific to the code changes, prioritized by impact, with confidence scores and exploit scenarios
...
| ------- | ------------ |
| **Manual (workflow_dispatch)** | Go to Actions > PR Security Review > Run workflow, paste the PR URL |
...
| **Reusable workflow** | Call `pr-review-reusable.yml@main` from your own pipeline — no files to copy, scripts are fetched automatically |
...
| `SDD_SLACK_WEBHOOK_URL` | No | Incoming webhook URL for review notifications |
| `LINEAR_API_KEY` | No | Linear API key for triage ticket creation on Required reviews |
| `LINEAR_TEAM_ID` | If using Linear | Linear team UUID for ticket creation |
| `LINEAR_TRIAGE_STATUS_ID` | If using Linear | Linear status UUID for the triage queue |

### 2. Choose How to Run

**Preferred — Reusable workflow (no files to copy):**

Call the canonical workflow directly from your pipeline. Scripts are fetched automatically from the practicum repo at run time:
...
If you need to customise the workflow or run it in an environment without access to the practicum repo, copy these files:
...
- **Exploit scenario** — a concrete, attacker-perspective description: what payload, what path, what preconditions, and what outcome.
...
- For PRs in a different org or private repo, pass `SOURCE_REPO_TOKEN` as a secret
